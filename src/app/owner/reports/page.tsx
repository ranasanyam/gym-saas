...
function RevenueTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-[hsl(220_25%_13%)] border border-white/10 rounded-xl px-3 py-2.5 shadow-xl text-xs">
      ...
      <p className="text-white font-bold border-t border-white/10 pt-1.5 mt-1.5">
        Total: {fmt(payload.reduce((s: number, p: any) => s + (p.value ?? 0), 0))}
      </p>
    </div>
  )
}

function SimpleTooltip({ active, payload, label, color, formatValue = fmt }: any) {
  if (!active || !payload?.length) return null
  return (
    <div className="bg-[hsl(220_25%_13%)] border border-white/10 rounded-xl px-3 py-2.5 shadow-xl text-xs">
      <p className="text-white/50 font-medium mb-1">{label}</p>
      <p style={{ color }}>{formatValue(payload[0]?.value ?? 0)}</p>
    </div>
  )
}
...
function StackedBar({ data }: { data: RevenueBucket[] }) {
  if (!data.length || data.every(d => d.total === 0))
    return <div className="h-36 flex items-center justify-center text-white/20 text-sm">No data for this period</div>

  const chartData = data.map(d => ({
    label:      d.label,
    Membership: d.membershipRev,
    Supplements: d.supplementRev,
    Lockers:    d.lockerRev,
  }))

  return (
    <ResponsiveContainer width="100%" height={160}>
      <BarChart data={chartData} barCategoryGap="20%" margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
        <CartesianGrid strokeDasharray="3 3" stroke={GRID_STROKE} vertical={false} />
        <XAxis dataKey="label" tick={AXIS_TICK} tickLine={false} axisLine={false} interval="preserveStartEnd" />
        <YAxis tickFormatter={fmt} tick={AXIS_TICK} tickLine={false} axisLine={false} width={54} />
        <Tooltip content={<RevenueTooltip />} cursor={{ fill: "rgba(255,255,255,0.04)" }} />
        <Bar dataKey="Membership"  stackId="a" fill={C.membership} />
        <Bar dataKey="Supplements" stackId="a" fill={C.supplement} />
        <Bar dataKey="Lockers"     stackId="a" fill={C.locker} radius={[3, 3, 0, 0]} />
      </BarChart>
    </ResponsiveContainer>
  )
}
...
// ── Area/line chart ────────────────────────────────────────────────────────────
function LineChart({
  data, valueKey, color, emptyMsg = "No data for this period", formatValue = fmt,
}: {
  data: any[]; valueKey: string; color: string; emptyMsg?: string; formatValue?: (n: number) => string
}) {
  if (data.length < 2) return (
    <div className="h-28 flex items-center justify-center text-white/20 text-xs">{emptyMsg}</div>
  )

  const gradId = `ag-${color.replace(/[^a-z0-9]/gi, "")}`
  const chartData = data.map(d => ({ label: d.label, value: d[valueKey] }))
  ...
  return (
    <ResponsiveContainer width="100%" height={140}>
      <AreaChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
        ...
        <XAxis dataKey="label" tick={AXIS_TICK} tickLine={false} axisLine={false} interval="preserveStartEnd" />
        <YAxis tickFormatter={fmt} tick={AXIS_TICK} tickLine={false} axisLine={false} width={54} />
        <Tooltip content={<SimpleTooltip color={color} formatValue={formatValue} />} cursor={{ stroke: "rgba(255,255,255,0.1)", strokeWidth: 1 }} />
        <Area
          dataKey="value"
          stroke={color}
          strokeWidth={2}
...
function DualLineChart({
  ...
  if (revenueSeries.length < 2) return (
    <div className="h-28 flex items-center justify-center text-white/20 text-xs">No data for this period</div>
  )
  ...
  const chartData = revenueSeries.map(d => ({
    label:    d.label,
    Revenue:  d.total,
    Expenses: expByLabel.get(d.label) ?? 0,
  }))
    ...
    <ResponsiveContainer width="100%" height={150}>
      <RLineChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
        ...
        <Line
          ...
          stroke={C.membership}
          strokeWidth={2}
          dot={{ r: 3, fill: C.membership, strokeWidth: 0 }}
        ...
        <Line
          ...
          strokeWidth={1.5}
          strokeDasharray="5 3"
          dot={{ r: 2.5, fill: C.expense, strokeWidth: 0 }}
          activeDot={{ r: 4, strokeWidth: 0 }}
        />
      </RLineChart>
    </ResponsiveContainer>
    ...
    <>
      ...
      <div className="space-y-6 print-target">
        ...
        <div className="grid lg:grid-cols-2 gap-5">
          ...
          <div className="bg-[hsl(220_25%_9%)] border border-white/6 rounded-2xl p-5">
            ...
            {loading
              ...
              : <LineChart data={data?.memberGrowthSeries ?? []} valueKey="count" color={C.members} formatValue={n => `${n} members`} />
            }
            {data && !loading && (
        ...
        <div className="bg-[hsl(220_25%_9%)] border border-white/6 rounded-2xl p-5">
          ...
          {loading
            ? <div className="h-20 bg-white/3 rounded-xl animate-pulse" />
            : <LineChart data={data?.attendanceSeries ?? []} valueKey="count" color={C.attendance} emptyMsg="No check-ins in this period" formatValue={n => `${n} check-ins`} />
          }
        </div>
