...
// // Defined OUTSIDE component so React never recreates it — prevents input focus loss
...
// // Safe date addition that won't overflow months (e.g. Jan 31 + 1mo → Feb 28)
// function addMonths(dateStr: string, months: number): string {
//   const [y, m, d] = dateStr.split("-").map(Number)
//   const date = new Date(y, m - 1 + months, d)
//   // If day overflowed (e.g. Mar 31 + 1 = May 1 instead of Apr 30), clamp to last day of target month
...
//   // Auto-calc end date when plan or start changes
//   useEffect(() => {
//     if (selectedPlan && form.startDate) {
//       setForm(p => ({ ...p, endDate: addMonths(form.startDate, selectedPlan.durationMonths) }))
//     }
//   }, [form.membershipPlanId, form.startDate])
...
//       // 2. Enroll as gym member
//       const memberRes = await fetch("/api/owner/members", {
//         method: "POST", headers: { "Content-Type": "application/json" },
//         body: JSON.stringify({
//           gymId: form.gymId, profileId: profileData.id,
//           membershipPlanId: form.membershipPlanId || null,
//           startDate: form.startDate,
//           heightCm: form.heightCm || null, weightKg: form.weightKg || null,
...
//       if (!memberRes.ok) throw new Error(memberData.error)

//       toast({ variant: "success", title: "Member added!", description: `${form.fullName} enrolled successfully.` })
//       router.push(`/owner/members/${memberData.id}`)
//     } catch (err: any) {
//       toast({ variant: "destructive", title: err.message ?? "Failed to add member" })
...
//             <div className="w-20 h-20 rounded-2xl bg-white/5 border-2 border-dashed border-white/15 flex flex-col items-center justify-center cursor-pointer hover:border-primary/40 transition-colors group">
//               <Upload className="w-5 h-5 text-white/25 group-hover:text-primary/60 transition-colors" />
//               <span className="text-white/25 text-xs mt-1 group-hover:text-primary/60 transition-colors">Photo</span>
//             </div>
//             <div>
...
//               <select value={form.gymId} onChange={e => setForm(p => ({ ...p, gymId: e.target.value, membershipPlanId: "" }))}
//                 className="w-full bg-[hsl(220_25%_11%)] border border-white/10 text-white rounded-xl px-4 h-11 text-sm focus:outline-none focus:border-primary">
//                 {gyms.map(g => <option key={g.id} value={g.id}>{g.name}</option>)}
//               </select>
//             </Field>
//             <Field label="Membership Plan">
//               <select value={form.membershipPlanId} onChange={e => setForm(p => ({ ...p, membershipPlanId: e.target.value }))}
//                 className="w-full bg-[hsl(220_25%_11%)] border border-white/10 text-white rounded-xl px-4 h-11 text-sm focus:outline-none focus:border-primary">
...
import { useRouter, useSearchParams } from "next/navigation"
import { useToast } from "@/hooks/use-toast"
import { Loader2, ArrowLeft, Upload, AlertTriangle } from "lucide-react"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { Button } from "@/components/ui/button"
    ...
    try {
      ...
      if (data.status === "CREATED") {
        toast({
          variant: "success",
          title: "Member added!",
          description: `${form.fullName} has been enrolled. A password setup email has been sent to them.`,
        })
      ...
      if (data.status === "ENROLLED") {
        toast({
          ...
          description: `${form.fullName} has been added to ${selectedGym?.name}.`,
        })
        router.push(`/owner/members/${data.id}`)
        return
      }
