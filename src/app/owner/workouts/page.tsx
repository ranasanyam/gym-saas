...
export default function WorkoutsPage() {
  ...
  const openEdit = (plan: Plan) => {
    setEditingPlan(plan)
    setForm({
      gymId: gyms[0]?.id ?? "", allGyms: false,
      memberId: plan.assignedMember?.id ?? "", freeForAll: plan.isGlobal,
      isTemplate: plan.isTemplate, title: plan.title, description: plan.description ?? "",
  ...
  const handleCancel = () => {
    setFormMode(null); setEditingPlan(null); setWeekPlan({})
    setForm({ ...blankForm, gymId: gyms[0]?.id ?? "" })
  }
