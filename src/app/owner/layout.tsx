...
import {
  ...
  Loader2, Tag, Gift, ShoppingBag, Lock, Zap,
} from "lucide-react"
...
function OwnerLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  const Sidebar = ({ mobile = false }: { mobile?: boolean }) => (
    <aside className={`${mobile ? "flex" : "hidden lg:flex"} flex-col w-[215px] min-h-screen bg-[hsl(220_25%_7%)] border-r border-white/5`}>
          ...
          {navItems.map(({ label, href, icon: Icon, featureKey }) => {
            ...
            return (
              <Link key={href} href={href} onClick={() => setSidebarOpen(false)}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all ${active ? "bg-primary text-white font-semibold shadow-lg shadow-primary/20"
                    : isLocked ? "text-white/25 hover:text-white/40 hover:bg-white/3"
                      : "text-white/50 hover:text-white hover:bg-white/5"
                  }`}>
                <Icon className="w-4 h-4 shrink-0" />
                <span className="flex-1">{label}</span>
                {isLocked && !active && <Lock className="w-3 h-3 shrink-0 opacity-50" />}
              </Link>
            )
          })}
        </nav>
...
//   ChevronRight, Loader2, Tag, Gift, ShoppingBag, Zap
// } from "lucide-react"
// import { Avatar } from "@/components/ui/Avatar"
