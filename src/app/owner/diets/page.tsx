...
export default function DietsPage() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([
      fetch(`/api/owner/diets${qs}`).then(r => r.json()),
      fetch("/api/owner/gyms").then(r => r.json())
    ]).then(([p,g]) => {
      setPlans(Array.isArray(p) ? p : [])
      if (Array.isArray(g)) setGyms(g)
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-white/6">
              ...
              <div className="flex gap-3">
                ...
                <Button type="submit" disabled={saving}
                  className="bg-linear-to-r from-primary to-orange-400 hover:opacity-90 text-white font-semibold h-10 text-sm px-7 min-w-33">
                  {saving
                    ? <Loader2 className="w-3.5 h-3.5 animate-spin" />
            ...
            return (
              ...
              >
                ...
                <div className="flex gap-1 pt-2 border-t border-white/5">
                  ...
                  <button onClick={() => setArchiveId(plan.id)} disabled={isDeleting}
