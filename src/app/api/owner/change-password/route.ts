...
export async function POST(req: NextRequest) {
  ...
  const profile = await prisma.profile.findUnique({ where: { id: session.user.id }, select: { passwordHash: true } })
  ...
  await prisma.profile.update({ where: { id: session.user.id }, data: { passwordHash: hash } })
