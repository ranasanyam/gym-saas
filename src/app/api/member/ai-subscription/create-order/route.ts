// POST /api/member/ai-subscription/create-order — Razorpay order for member AI subscription
...
export async function POST(req: NextRequest) {
  ...
  if (activeSub) {
    ...
    // Upgrade allowed — subscribe endpoint will deactivate the old sub
  }
  ...
  try {
    order = await razorpay.orders.create({
      ...
      currency: "INR",
      receipt:  `aisub_${profileId.slice(0, 8)}_${Date.now()}`,
