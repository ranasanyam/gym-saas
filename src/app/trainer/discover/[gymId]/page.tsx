// src/app/trainer/discover/[gymId]/page.tsx
"use client"
...
export default function TrainerGymDiscoverDetailPage() {
  ...
  if (!gym) return (
    <div className="text-center py-20">
      ...
      <Link href="/trainer/discover" className="text-primary hover:underline text-sm mt-2 inline-block">← Back to Discover</Link>
    </div>
  )
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {gym.isJoined && (
                <span className="inline-flex items-center gap-1 text-xs px-2.5 py-0.5 rounded-full font-medium bg-green-500/15 text-green-400">
                  <CheckCircle2 className="w-3 h-3" /> Joined Gym
                </span>
              )}
            </div>
            ...
            <div className="flex items-center gap-4 mt-2 flex-wrap text-xs text-white/35">
              <span className="flex items-center gap-1"><Users className="w-3 h-3" />{gym._count?.trainers ?? 0} trainers</span>
              <span className="flex items-center gap-1"><Users className="w-3 h-3" />{gym._count?.members ?? 0} members</span>
              {contactNumber && (
