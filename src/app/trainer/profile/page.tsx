...
import { Lock, Save, Loader2, Eye, EyeOff, Camera } from "lucide-react"
...
export default function TrainerProfilePage() {
  ...
  const changePassword = async () => {
    ...
    if (res.ok) { toast({ variant: "success", title: "Password changed" }); setPwForm({ current: "", next: "", confirm: "" }) }
    ...
    <div className="max-w-xl space-y-5">
      ...
      <div className="bg-[hsl(220_25%_9%)] border border-white/6 rounded-2xl p-6 flex items-center gap-5">
        <label className="relative cursor-pointer group shrink-0">
          ...
          {profile?.avatarUrl ? (
            <img src={profile.avatarUrl} alt={profile.fullName} className="w-16 h-16 rounded-2xl object-cover" />
          ) : (
            <div className="w-16 h-16 rounded-2xl bg-gradient-primary flex items-center justify-center text-white text-2xl font-display font-bold">
              {initials}
            </div>
          )}
        ...
        <div>
          ...
          {profile?.gym && <p className="text-white/30 text-xs mt-0.5">{profile.gym.name}</p>}
          <p className="text-white/30 text-xs mt-1.5">Click photo to change</p>
          <span className="inline-block mt-1 text-xs bg-purple-500/15 text-purple-400 px-3 py-1 rounded-full">Trainer</span>
        </div>
      </div>
