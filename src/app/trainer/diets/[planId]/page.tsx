// src/app/trainer/diets/[planId]/page.tsx
"use client"
...
export default function TrainerDietDetailPage() {
  ...
  useEffect(() => {
    fetch(`/api/trainer/diets/${planId}`)
      .then(r => r.ok ? r.json() : Promise.reject())
      .then(setPlan)
      .catch(() => router.replace("/trainer/diets"))
  ...
  const archive = async () => {
    ...
    setArchiving(true)
    const res = await fetch(`/api/trainer/diets/${planId}`, { method: "DELETE" })
    setShowConfirmModal(false);
    if (res.ok) {
      toast({ variant: "success", title: "Plan archived" })
      router.push("/trainer/diets")
    } else {
    ...
    <div className="max-w-3xl space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => router.push(`/trainer/diets?edit=${planId}`)}
