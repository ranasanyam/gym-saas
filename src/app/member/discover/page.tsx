...
import {
  ...
  ChevronRight, Loader2, CheckCircle2, Phone, Image as ImageIcon
} from "lucide-react"
...
export default function DiscoverGymsPage() {
  ...
  const load = (s: string, c: string) => {
    ...
    fetch(`/api/member/discover?${params}`)
      ...
      .then(d => {
        setGyms(d.gyms ?? [])
        if (d.memberCity && !city) setCity(d.memberCity ?? "")
        setMemberCity(d.memberCity ?? "")
      })
      .finally(() => setLoading(false))
  }
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {gyms.map(gym => {
              ...
              return (
                <div
                  key={gym.id}
                  onClick={() => router.push(`/member/discover/${gym.id}`)}
                    ...
                    <div className="flex items-center gap-3 flex-wrap text-xs mb-3">
                      ...
                      {cheapestPlan && (
                        <span className="text-primary font-semibold">
                          from ₹{Number(cheapestPlan.price).toLocaleString("en-IN")}/mo
                        </span>
                      )}
                      {contactNumber && (
