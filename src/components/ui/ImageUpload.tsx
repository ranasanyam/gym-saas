...
interface Props {
  ...
  size?:       number
  placeholder?: string
  // Which Cloudinary folder to upload to
...
export function ImageUpload({
  ...
  const borderRadius = shape === "circle" ? "9999px" : "12px"
  const style        = { width: size, height: size, borderRadius, flexShrink: 0 as const }
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <button
        ...
        disabled={uploading}
        className="flex flex-col items-center justify-center bg-white/5 border-2 border-dashed border-white/15 hover:border-primary/40 transition-colors cursor-pointer disabled:cursor-not-allowed"
        style={style}
      >
        {uploading
          ? <Loader2 className="w-5 h-5 text-primary animate-spin" />
          : (
            <>
              ...
              <span className="text-white/25 text-[10px]">{placeholder}</span>
            </>
          )
        }
      </button>
      {error && <p className="text-red-400 text-[10px] max-w-30">{error}</p>}
    </div>
  )
}